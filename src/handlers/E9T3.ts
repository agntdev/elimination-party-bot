...
composer.callbackQuery("join:round", async (ctx) => {
  ...
  if (!ctx.chat || !ctx.from) {
    await ctx.editMessageText("Unable to join: missing chat or user details.");
    return;
  }
  ...
  try {
    ...
    if (result.status === "insufficient_balance") {
      await ctx.editMessageText(
        `Not enough points! Current balance: ${result.balance}`,
      );
      return;
    }
    ...
    if (isGameStorageConfigError(err)) {
      await ctx.editMessageText(
        "Round storage is not configured. Set REDIS_URL before joining rounds.",
      );
      return;
    }
    throw err;
