# fix-1536b645d2d5be8d — Use Redis for durable game storage

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 ELIM

Merged game persistence requires PostgreSQL via `DATABASE_URL`, but the agnt bot runtime provides Redis via `REDIS_URL`. Joining rounds fails in deployment with `Round storage is not configured. Set DATABASE_URL before joining rounds.` Move durable group/player/round/transaction state to Redis, update storage messages/specs/docs to `REDIS_URL`, and remove the unused PostgreSQL runtime dependency.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-1536b645d2d5be8d.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-1536b645d2d5be8d.json` (and any new command `tests/commands/fix-1536b645d2d5be8d.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-1536b645d2d5be8d.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-1536b645d2d5be8d.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
