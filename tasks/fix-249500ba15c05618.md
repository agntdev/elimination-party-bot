# fix-249500ba15c05618 — E10T2: Auto-start is poll-based, not timer-based (spec non-compliance)

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 ELIM

The E10T2 spec states: *"The countdown should begin automatically after join_window_seconds if no manual start occurs. Verify timer logic and state transitions from 'open' to 'countdown.'"* The implementation relies on `composer.on("message", ...)` — the expiry check only runs when someone sends a group message. If no message arrives after the join window expires, the round never auto-starts.

While `RedisGameRepository.autoStartIfJoinWindowExpired()` correctly computes the expiry time (`src/game/redis-repository.ts:248-252`), the trigger mechanism is poll-based rather than timer-based. A durable timer (e.g. `setTimeout` with recovery-on-restart from stored expiry times) is needed to meet the *"begin automatically"* requirement.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-249500ba15c05618.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-249500ba15c05618.json` (and any new command `tests/commands/fix-249500ba15c05618.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-249500ba15c05618.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-249500ba15c05618.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
