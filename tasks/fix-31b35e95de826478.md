# fix-31b35e95de826478 — E9T2: Run Round Now button visible to all users, not only the group creator

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 ELIM

E9T2 spec explicitly states: "The 'Run Round Now' button should only be visible to the group creator." The inline result produced by `startGameInlineResult()` in `E9T1.ts:26-28` includes the Run Round Now button unconditionally for every user. While `E9T4.ts:28-31` correctly rejects non-creators who click it (returning "not_creator"), the button should not appear at all for non-creators. The `chosen_inline_result` event provides `result.from` but does not directly expose the group chat ID to check creator status — this requires either storing a mapping from `inline_message_id` → groupId at selection time, or accepting that the button appears to all but is guarded by callback authorization.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-31b35e95de826478.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-31b35e95de826478.json` (and any new command `tests/commands/fix-31b35e95de826478.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-31b35e95de826478.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-31b35e95de826478.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
