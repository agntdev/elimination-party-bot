# fix-3e1c2230ff709ba4 — E9T2 chosen_inline_result uses hardcoded DEFAULT_STAKE=10

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 ELIM

`E9T2.ts:5` defines `DEFAULT_STAKE = 10` and uses it on line 16: `\`Stake: ${DEFAULT_STAKE} points\``. The actual group stake may differ (set via `/setstake`). The `chosen_inline_result` update lacks chat context so the bot cannot look up the real group setting at this point — but the displayed message is potentially incorrect. Consider either omitting the stake value or noting it's the default.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-3e1c2230ff709ba4.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-3e1c2230ff709ba4.json` (and any new command `tests/commands/fix-3e1c2230ff709ba4.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-3e1c2230ff709ba4.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-3e1c2230ff709ba4.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
