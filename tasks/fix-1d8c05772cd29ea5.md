# fix-1d8c05772cd29ea5 — E9T4: silent no-op when startRound returns no_open_round

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 ELIM

In `E9T4.ts:24-26`, when `repository.startRound()` returns `status: "no_open_round"`, the handler simply `return`s without calling `ctx.editMessageText()` or any other user-visible feedback. `ctx.answerCallbackQuery()` was already called on line 10, so the loading spinner stops, but the user sees nothing change and gets no explanation. This can happen if another user already started the round, or if the round was cancelled between the inline result being posted and this callback firing.

Fix: add an `ctx.editMessageText("There is no open round to start. The round may have already been started or cancelled.")` (or similar) before returning.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-1d8c05772cd29ea5.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-1d8c05772cd29ea5.json` (and any new command `tests/commands/fix-1d8c05772cd29ea5.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-1d8c05772cd29ea5.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-1d8c05772cd29ea5.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
