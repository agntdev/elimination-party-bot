# fix-2c4e04544c2d9070 — E9T4 silently drops no_open_round without user feedback

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 ELIM

In `E9T4.ts:24-26`, when `startRound` returns `status: "no_open_round"`, the handler **returns without any message**. The user only sees the `answerCallbackQuery` toast and gets no explanation. Compare `E1T3.ts:29-32` which shows `"No open round to start. Tap Join first."` for the same condition. A user clicking "Run Round Now" with no open round would be confused by the silent failure.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-2c4e04544c2d9070.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-2c4e04544c2d9070.json` (and any new command `tests/commands/fix-2c4e04544c2d9070.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-2c4e04544c2d9070.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-2c4e04544c2d9070.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
