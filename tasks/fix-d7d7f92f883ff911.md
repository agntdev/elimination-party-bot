# fix-d7d7f92f883ff911 — E10T2: Auto-start message leaks into fallback handler (double-message bug)

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 ELIM

When `autoStartIfJoinWindowExpired` triggers a round start (sends reply + countdown + elimination), the handler falls through to `await next()` on line 45. This routes the originating message to the fallback handler in `bot.ts:48`, which replies "*Sorry, I didn't understand that.*". The user sees game-start messages **and** the fallback message interleaved — confirmed by the test spec (`tests/specs/E10T2.json` lines 35-42) which expects this output.

**Root cause:** lines 34–36 handle the auto-start but never `return`; the function body continues to line 45 unconditionally. Fix: add `return;` after line 36 (or restructure so `next()` is only called when the handler did NOT process the message). The associated test spec also needs updating to remove the fallback message from expected output.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-d7d7f92f883ff911.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-d7d7f92f883ff911.json` (and any new command `tests/commands/fix-d7d7f92f883ff911.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-d7d7f92f883ff911.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-d7d7f92f883ff911.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
